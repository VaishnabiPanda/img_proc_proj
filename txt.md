# Project Notes - Image Defect Classification

## Current Project Layout

```text
img_proc_proj/
|-- app.py                       # Flask web server; loads the trained model and serves /predict
|-- train.py                     # ResNet50 training pipeline for MVTec AD good/bad classes
|-- index.html                   # Browser UI for image upload and prediction results
|-- requirements.txt             # Python dependencies
|-- labels.json                  # Label mapping used by the app/model
|-- summary.md                   # Main project summary and documentation
|-- txt.md                       # Training history and project notes
|-- training_dashboard.png       # Generated training report image; ignored by git
|-- best_exact_class_model.pth   # Trained model weights; ignored by git
|-- archive/                     # Local MVTec AD dataset; ignored by git
|-- archivezip.zip               # Local dataset archive; ignored by git
|-- venv/                        # Local virtual environment; ignored by git
|-- __pycache__/                 # Python cache files; ignored by git
`-- .gitignore                   # Prevents large/local files from being uploaded
```

## GitHub Upload Notes

This project is ready to upload to GitHub as source code. The `.gitignore` excludes local and large files that should not be committed:
...
Important files that should stay local or be shared separately:

- `archivezip.zip` is about 5.27 GB, so it must not be uploaded to a normal GitHub repository.
- `archive/` contains the dataset and should stay local.
- `best_exact_class_model.pth` is about 99 MB. It is close to GitHub's normal file limit and is currently ignored. Share it through GitHub Releases, Google Drive, or Git LFS if needed.
- `venv/` should never be uploaded. Anyone using the project can recreate it from `requirements.txt`.

Recommended GitHub upload commands after installing Git:

```powershell
cd "C:\Users\KIIT\Desktop\img_proc_proj"
git init
git add .
...
Only source files such as `app.py`, `train.py`, `index.html`, `requirements.txt`, `labels.json`, `summary.md`, and `txt.md` should appear in the commit.
